...
- **เว็บ Dashboard (FastAPI):**
  - **ภาพรวม** (`/?month=YYYY-MM`) และ **รายละเอียดต่อธนาคาร** — เปิดดูได้สาธารณะ ไม่ต้อง login
    ...
    - รายละเอียด: กราฟแนวโน้ม (วาดเป็น SVG เอง ไม่พึ่งไลบรารี, วันที่แสดงเป็น พ.ศ.) + สรุปรายเดือนแบบเดียวกับภาพรวม + ลิงก์เปิด PDF ย้อนหลังจัดกลุ่มตามปี + แถบเตือนสีแดงเมื่อประกาศล่าสุดมีค่าที่อ่านไม่ได้
  - **จัดการอัตรา**, **แก้ค่า (manual)** และ **Log & รัน** — สงวนไว้เฉพาะผู้ดูแลที่ login แล้วเท่านั้น
    ...
    - แก้ค่า (`/bank/{code}/manual`): กรอกค่าที่ OCR/parser อ่านผิดหรืออ่านไม่ได้ทับทีละช่อง (ช่องว่าง = ค่าที่หายไป) บันทึกแล้ว trigger `--backfill` ให้เองอัตโนมัติ
